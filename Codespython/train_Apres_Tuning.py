...
def train_model(data_yaml_path, project_dir, run_name):
    ...
    try:
        ...
        results = model.train(
            data=data_yaml_path,
            cfg="P:\Videos\projet\Resultats_Entrainements_Tuning\TUNING_FINAL_SESSION5/best_hyperparameters.yaml", # <--- ON AJOUTE ÇA
            project=project_dir,
            name=run_name,
            epochs=epochs,
            patience=5,
...
if __name__ == "__main__":
    ...
    experiments = [
        ...
        (os.path.join(yaml_dir, "Data_Complet_Augmente.yaml"), "Complet_Augmente.yaml_tuning"),
        (os.path.join(yaml_dir, "data_v2.yaml"), "Base_tuning"),
        (os.path.join(yaml_dir, "data_v2_perspective.yaml"), "Perpective_tuning"),
        (os.path.join(yaml_dir, "data_v2_hsv_rand.yaml"), "Hsv_rand_tuning"),
        (os.path.join(yaml_dir, "data_v2_rotation.yaml"), "Rotation_tuning"),
        (os.path.join(yaml_dir, "data_motion_blur.yaml"), "Motion_blur_tuning"),
        (os.path.join(yaml_dir, "data_lowres.yaml"), "Lowres_tuning"),
        (os.path.join(yaml_dir, "data_distortion.yaml"), "Distortion_tuning")
