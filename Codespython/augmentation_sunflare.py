...
def appliquer_sunflare(image, bboxes, class_labels):
    """
    Applique l'effet de reflet solaire via Albumentations.
    """
    transform = A.Compose([
        A.RandomSunFlare(
            p=1.0, 
            flare_roi=(0, 0, 1, 0.5), # Limite le reflet a la moitie haute de l'image
            angle_lower=0.5
        )
...
def augment_sunflare_dataset(img_dir, label_dir, output_dir):
    out_images = Path(output_dir) / "images"
    out_labels = Path(output_dir) / "labels"
    out_images.mkdir(parents=True, exist_ok=True)
    out_labels.mkdir(parents=True, exist_ok=True)

    print(f"--- Debut de l'augmentation : SunFlare -> {output_dir} ---")
    ...
    for label_file in Path(label_dir).glob("*.txt"):
        ...
        # 2. VERSION AVEC REFLET (SUNFLARE)
        try:
            img_flare, boxes_flare, labels_flare = appliquer_sunflare(img, bboxes, class_labels)
            
            # Sauvegarde Image
            cv2.imwrite(str(out_images / f"{label_file.stem}_flare.jpg"), img_flare)
            
            # Sauvegarde Labels
            with open(out_labels / f"{label_file.stem}_flare.txt", "w") as f:
                for box, cls in zip(boxes_flare, labels_flare):
