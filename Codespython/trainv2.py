...
def plot_history_yolo(save_dir):
    """Lit le fichier results.csv et génère des graphiques personnalisés."""
    csv_path = os.path.join(save_dir, 'results.csv')
    if not os.path.exists(csv_path):
        print(f"⚠️ Fichier de résultats introuvable : {csv_path}")
        return
...
def train_model(data_yaml_path, project_dir, run_name):
    """Entraîne YOLO et valide sur le set de TEST final."""
    epochs = 200
    batch = 8
    workers = 0
    ...
    try:
        ...
        print(f"\n🔬 ÉVALUATION SUR LE SPLIT TEST : {run_name}")
        ...
        print(f"✅ Résultats Test ({run_name}) - mAP50: {test_results.box.map50:.4f}")
        ...
        print(f"❌ Erreur lors de l'exécution de {run_name} : {e}")
...
if __name__ == "__main__":
    ...
    experiments = [
        (os.path.join(yaml_dir, "data.yaml"), "BaseLine"),
        (os.path.join(yaml_dir, "data_Zoom.yaml"), "Augmentation_Zoom"),
        (os.path.join(yaml_dir, "data_rot.yaml"), "Augmentation_Rotation"),
        (os.path.join(yaml_dir, "data_light_rand.yaml"), "Augmentation_Light_Random"),
        (os.path.join(yaml_dir, "data_perspective.yaml"), "Augmentation_Perspective"),
        (os.path.join(yaml_dir, "data_blur.yaml"), "Augmentation_Blur")
    ]
    ...
    for yaml_path, run_name in experiments:
        if os.path.exists(yaml_path):
            print(f"\n--- Début de l'expérience {entrainements_termines + 1}/{total_prevu} : {run_name} ---")
    ...
    print(f"\n📊 Résumé : {entrainements_termines}/{total_prevu} entraînements effectués.")

    if entrainements_termines > 0:
        print("✅ Tous les modèles ont été traités. Lancement du script de fin dans 10 secondes...")
        ...
        print("⚠️ Aucun entraînement n'a été lancé (fichiers YAML introuvables). Le script de fin est annulé.")