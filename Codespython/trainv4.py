...
# BASE_DIR = dossier 'Codespython'
BASE_DIR = Path(__file__).resolve().parent
# parent_dir = dossier racine 'IA-Lunette-Inteligente'
parent_dir = BASE_DIR.parent

def plot_history_yolo(save_dir):
    # Conversion du chemin en objet Path au cas ou
...
def train_model(data_yaml_path, project_dir, run_name, unfreeze_at_epoch):
    ...
    print(f"START: {run_name} | Device: {device} | Degel a l'epoque: {unfreeze_at_epoch}")
    
    model = YOLO(parent_dir / "yolo11m.pt")

    def unfreeze_callback(trainer):
        if trainer.epoch == unfreeze_at_epoch:
            print(f"INFO: Degel du modele a l'epoque {trainer.epoch}. Activation de toutes les couches.")
    ...
    try:
        results = model.train(
            data=str(data_yaml_path), # YOLO prefere parfois les strings
        ...
        print(f"EVALUATION TEST : {run_name}")
        ...
        # ### AJOUT SECURITE : Nettoyage final pour l'experience suivante
        del model
        gc.collect()
        if torch.cuda.is_available(): torch.cuda.empty_cache()
        
if __name__ == "__main__":
    # Utilisation des chemins relatifs a la racine du projet
    base_folder = parent_dir / "Dataset_Basev2" / "Resultats_Entrainements"
    base_folder.mkdir(parents=True, exist_ok=True)
    yaml_dir = parent_dir / "yamlFiles"

    # Liste des experiences (Chemin du YAML, Nom du run, Epoque de degel)
    experiments = [
    ...
    for yaml_path, run_name, unfreeze_epoch in experiments:
        if yaml_path.exists():
            ...
            print(f"ERREUR : Fichier YAML introuvable : {yaml_path}")

    print(f"\nResume : {termines}/{len(experiments)} entrainements effectues.")
    
    if termines > 0:
        print("Fin du script. Fermeture dans 10 secondes...")
        time.sleep(10)
        # Appel du script .bat de fin (si present a la racine)
