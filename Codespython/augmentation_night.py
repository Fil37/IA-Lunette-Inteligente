...
def appliquer_nuit_bruitee(image, bboxes, class_labels):
    """
    Simule une scene de nuit avec du bruit capteur.
    """
    transform = A.Compose([
        # Reduction de la luminosite (entre -0.5 et -0.2)
        A.RandomBrightnessContrast(brightness_limit=(-0.5, -0.2), contrast_limit=0, p=1.0),
...
def augment_dark_noise_dataset(img_dir, label_dir, output_dir):
    out_images = Path(output_dir) / "images"
    out_labels = Path(output_dir) / "labels"
    out_images.mkdir(parents=True, exist_ok=True)
    out_labels.mkdir(parents=True, exist_ok=True)

    print(f"--- Debut de l'augmentation : Nuit + Bruit -> {output_dir} ---")
    ...
    for label_file in Path(label_dir).glob("*.txt"):
        ...
        # 2. Version SOMBRE ET BRUITEE
        try:
            img_aug, boxes_aug, labels_aug = appliquer_nuit_bruitee(img, bboxes, class_labels)
            
            # Sauvegarde Image avec suffixe _darknoise
            cv2.imwrite(str(out_images / f"{label_file.stem}_darknoise.jpg"), img_aug)
            
            # Sauvegarde Labels avec suffixe _darknoise
            with open(out_labels / f"{label_file.stem}_darknoise.txt", "w") as f:
                for box, cls in zip(boxes_aug, labels_aug):
