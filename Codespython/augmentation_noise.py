...
def appliquer_bruit(image, bboxes, class_labels):
    """
    Applique un bruit gaussien via Albumentations.
    """
    transform = A.Compose([
        A.GaussNoise(
            var_limit=(10.0, 50.0), # Intensite du bruit (min, max)
...
def augment_noise_dataset(img_dir, label_dir, output_dir):
    out_images = Path(output_dir) / "images"
    out_labels = Path(output_dir) / "labels"
    out_images.mkdir(parents=True, exist_ok=True)
    out_labels.mkdir(parents=True, exist_ok=True)

    print(f"--- Debut de l'augmentation : Bruit Gaussien -> {output_dir} ---")
    ...
    for label_file in Path(label_dir).glob("*.txt"):
        ...
        # 2. Version avec BRUIT
        try:
            img_noise, boxes_noise, labels_noise = appliquer_bruit(img, bboxes, class_labels)
            
            # Sauvegarde Image avec suffixe _noise
            cv2.imwrite(str(out_images / f"{label_file.stem}_noise.jpg"), img_noise)
            
            # Sauvegarde Labels avec suffixe _noise
            with open(out_labels / f"{label_file.stem}_noise.txt", "w") as f:
                for box, cls in zip(boxes_noise, labels_noise):
